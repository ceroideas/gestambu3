...
/* Listado Málaga */
$lstMa = mysqli_query($gestambu, "SELECT servicio.idCia, servicio.provincia, servicio.tipo, DATE_FORMAT(servicio.fecha, '%d-%m-%y') AS fecha, servicio.estServ,
  ...
  WHERE servicio.fecha = '$diaIni' AND servicio.estServ NOT IN('15', '16') AND servicio.provincia = '29'
  GROUP BY servicio.IdCia
  ");

/* Listado Sevilla */
$lstSe = mysqli_query($gestambu, "SELECT servicio.idCia, servicio.provincia, servicio.tipo, DATE_FORMAT(servicio.fecha, '%d-%m-%y') AS fecha, servicio.estServ,
      SUM(IF(servicio.recurso = '1', 1, 0)) AMB,
      SUM(IF(servicio.recurso = '2', 1, 0)) DUE,
      SUM(IF(servicio.recurso = '3', 1, 0)) UVI,
      SUM(IF(servicio.recurso = '4', 1, 0)) VM,
  ...
  FROM servicio
    LEFT JOIN cia ON servicio.idCia = cia.idCia
  WHERE servicio.fecha = '$diaIni' AND servicio.estServ NOT IN('15', '16') AND servicio.provincia = '41'
  GROUP BY servicio.IdCia
  ");

/* Listado Cádiz */
$lstCa = mysqli_query($gestambu, "SELECT servicio.idCia, servicio.provincia, servicio.tipo, DATE_FORMAT(servicio.fecha, '%d-%m-%y') AS fecha, servicio.estServ,
      SUM(IF(servicio.recurso = '1', 1, 0)) AMB,
      SUM(IF(servicio.recurso = '2', 1, 0)) DUE,
      SUM(IF(servicio.recurso = '3', 1, 0)) UVI,
      SUM(IF(servicio.recurso = '4', 1, 0)) VM,
  ...
  FROM servicio
    LEFT JOIN cia ON servicio.idCia = cia.idCia
  WHERE servicio.fecha = '$diaIni' AND servicio.estServ NOT IN('15', '16') AND servicio.provincia = '11'
  GROUP BY servicio.IdCia
  ");
  
/* Listado Huelva */
$lstHu = mysqli_query($gestambu, "SELECT servicio.idCia, servicio.provincia, servicio.tipo, DATE_FORMAT(servicio.fecha, '%d-%m-%y') AS fecha, servicio.estServ,
      SUM(IF(servicio.recurso = '1', 1, 0)) AMB,
      SUM(IF(servicio.recurso = '2', 1, 0)) DUE,
      SUM(IF(servicio.recurso = '3', 1, 0)) UVI,
      SUM(IF(servicio.recurso = '4', 1, 0)) VM,
  ...
  FROM servicio
    LEFT JOIN cia ON servicio.idCia = cia.idCia
  WHERE servicio.fecha = '$diaIni' AND servicio.estServ NOT IN('15', '16') AND servicio.provincia = '21'
  GROUP BY servicio.IdCia
  ");

/* Listado Córdoba */
$lstCo = mysqli_query($gestambu, "SELECT servicio.idCia, servicio.provincia, servicio.tipo, DATE_FORMAT(servicio.fecha, '%d-%m-%y') AS fecha, servicio.estServ,
      SUM(IF(servicio.recurso = '1', 1, 0)) AMB,
      SUM(IF(servicio.recurso = '2', 1, 0)) DUE,
      SUM(IF(servicio.recurso = '3', 1, 0)) UVI,
      SUM(IF(servicio.recurso = '4', 1, 0)) VM,
  ...
  FROM servicio
    LEFT JOIN cia ON servicio.idCia = cia.idCia
  WHERE servicio.fecha = '$diaIni' AND servicio.estServ NOT IN('15', '16') AND servicio.provincia = '14'
  GROUP BY servicio.IdCia
  ");

/* Listado Melilla */
$lstMe = mysqli_query($gestambu, "SELECT servicio.idCia, servicio.provincia, servicio.tipo, DATE_FORMAT(servicio.fecha, '%d-%m-%y') AS fecha, servicio.estServ,
      SUM(IF(servicio.recurso = '1', 1, 0)) AMB,
      SUM(IF(servicio.recurso = '2', 1, 0)) DUE,
      SUM(IF(servicio.recurso = '3', 1, 0)) UVI,
      SUM(IF(servicio.recurso = '4', 1, 0)) VM,
  ...
  FROM servicio
    LEFT JOIN cia ON servicio.idCia = cia.idCia
  WHERE servicio.fecha = '$diaIni' AND servicio.estServ NOT IN('15', '16') AND servicio.provincia = '52'
  GROUP BY servicio.IdCia
  ");


/* Listado Completo */
$lsTotal = mysqli_query($gestambu, "SELECT servicio.idCia, servicio.provincia, servicio.tipo, DATE_FORMAT(servicio.fecha, '%d-%m-%y') AS fecha, servicio.estServ,
      SUM(IF(servicio.recurso = '1', 1, 0)) AMB,
      SUM(IF(servicio.recurso = '2', 1, 0)) DUE,
      SUM(IF(servicio.recurso = '3', 1, 0)) UVI,
      SUM(IF(servicio.recurso = '4', 1, 0)) VM,
  ...
  FROM servicio
    LEFT JOIN cia ON servicio.idCia = cia.idCia
  WHERE servicio.fecha = '$diaIni' AND servicio.estServ NOT IN('15', '16')
  GROUP BY servicio.recurso
  ");
echo mysqli_error($gestambu);
var_dump($lsTotal);
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li>Estadísticas</li>
        <li class="active">Servicios diarios</li>
      </ol>
    </section>
    ...
    <section class="content">
                ...
                <form class="form-vertical" action="" method="POST">
                  <div class="box-body">
                    ...
                    <div class="form-group col-md-3 col-sm-3 col-xs-4">
                      <label> &nbsp; </label>
                      <div class="input-group">
                        <button type="submit" class="btn btn-default" value='enviar' name='consulta'>Consultar</button>
                      </div>
                    </div>
                  </div>
                  <!-- /.box-footer -->
      ...
      <div class="box">
        <div class="box-header">
          <h3 class="box-title">Resultados: <?php if(@$_POST['consulta'] == 'enviar') { echo date_format(date_create("$diaIni"), 'd-m-Y'); } ?> </h3>
        </div>
        <!-- /.box-header -->
        <div class="box-body table-responsive no-padding">
          <table class="table table-conndensed">
            ...
            <tr>
              ...
              <td><?php echo $rwCa['AMB']; ?></td>
              <td><?php echo $rwCa['UVI']; ?></td>
              <td><?php echo $rwCa['VM']; ?></td>
              <td><?php echo $rwCa['DUE']; ?></td>
              <td><?php echo $rwCa['TAXI']; ?></td>
			  <td><?php echo $rwCa['RUTA']; ?></td>
            ...
            <tr>
              ...
              <td><?php echo $rwMa['AMB']; ?></td>
              <td><?php echo $rwMa['UVI']; ?></td>
              <td><?php echo $rwMa['VM']; ?></td>
              <td><?php echo $rwMa['DUE']; ?></td>
              <td><?php echo $rwMa['TAXI']; ?></td>
			  <td><?php echo $rwMa['RUTA']; ?></td>
            ...
            <tr>
              ...
              <td><?php echo $rwHu['AMB']; ?></td>
              <td><?php echo $rwHu['UVI']; ?></td>
              <td><?php echo $rwHu['VM']; ?></td>
              <td><?php echo $rwHu['DUE']; ?></td>
              <td><?php echo $rwHu['TAXI']; ?></td>
			  <td><?php echo $rwHu['RUTA']; ?></td>
            ...
            <tr>
              ...
              <td><?php echo $rwSe['AMB']; ?></td>
              <td><?php echo $rwSe['UVI']; ?></td>
              <td><?php echo $rwSe['VM']; ?></td>
              <td><?php echo $rwSe['DUE']; ?></td>
              <td><?php echo $rwSe['TAXI']; ?></td>
			  <td><?php echo $rwSe['RUTA']; ?></td>
            ...
            <tr>
              <td></td>
              <td><?php echo $rwTotal['AMB']; ?></td>
              <td><?php echo $rwTotal['UVI']; ?></td>
              <td><?php echo $rwTotal['VM']; ?></td>
              <td><?php echo $rwTotal['DUE']; ?></td>
              <td><?php echo $rwTotal['TAXI']; ?></td>
			  <td><?php echo $rwTotal['RUTA']; ?></td>
