...
if(isset($_GET['prov'])) {
  $provincia = "'".$_GET['prov']."'";
  /*if($provincia == "'41'") {
	  $provincia = "'41','21'"; //se evita la inclusion de los servicios de huelva en sevilla por peticion se comenta este bloque
  } else {
	  $provincia = $provincia;
  }*/
} else {
...
if($recuSel == 1) {
  ...
  $listLibreta = mysqli_query($gestambu, "SELECT servicio.idSv, servicio.continuado, servicio.idCia, servicio.provincia, servicio.tipo, servicio.recurso, servicio.fecha, servicio.hora, servicio.medico, servicio.enfermero, servicio.idvta, servicio.nombre, servicio.orden,
    ...
    WHERE servicio.fecha = '$diaMostrar' AND servicio.provincia IN($provincia) AND servicio.recurso IN('1', '3', '5') AND estServ NOT IN('3','10','14','15','16')
  ...
  $listLibreta = mysqli_query($gestambu, "SELECT servicio.idSv, servicio.continuado, servicio.idCia, servicio.provincia, servicio.tipo, servicio.recurso, servicio.fecha, servicio.hora, servicio.enfermero, servicio.nombre, servicio.orden, servicio.obs,
    ...
    FROM servicio
      ...
      LEFT JOIN serestados ON servicio.idSv = serestados.idSv
      LEFT JOIN vehiculo ON serestados.vhIda = vehiculo.idVh
    WHERE servicio.fecha = '$diaMostrar' AND servicio.provincia IN($provincia) AND servicio.recurso = '2' AND estServ NOT IN('3','10','14','15','16')
  ...
  $listLibreta = mysqli_query($gestambu, "SELECT servicio.idSv, servicio.continuado, servicio.idCia, servicio.provincia, servicio.tipo, servicio.recurso, servicio.fecha, servicio.hora, servicio.enfermero, servicio.nombre, servicio.orden, servicio.obs,
    ...
    FROM servicio
      ...
      LEFT JOIN serestados ON servicio.idSv = serestados.idSv
      LEFT JOIN vehiculo ON serestados.vhIda = vehiculo.idVh
    WHERE servicio.fecha = '$diaMostrar' AND servicio.provincia IN($provincia) AND servicio.recurso = '7' AND estServ NOT IN('3','10','14','15','16')
    ORDER BY servicio.codRuta, servicio.orden, servicio.hora, servicio.nombre ASC
    ");	
}
...
<head>
  ...
  <title>GestAmbu 3.0 | Libreta de: <?php provValor($_GET['prov']); ?>-<?php if($recuSel == '1') { echo "Ambulancia"; } elseif($recuSel == 2) { echo "Enfermero";} ?></title>
  <!-- Ancho de pantalla -->
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li>Servicios</li>
        <li class="active">Libreta: <?php provValor($_GET['prov']); ?></li>
      </ol>
    </section>
    ...
    <section class="content">
          ...
          <div class="box box-danger">
            <div class="box-header with-border">
              <h3 class="box-title">
                ...
                <strong><?php provValor($_GET['prov']); ?>-<i><?php if($recuSel == '1') { echo "Ambulancia"; } elseif($recuSel == 2) { echo "Enfermero";} ?></i></strong>
              </h3>
                  ...
                  <div class="box-body">
                    ...
                    <div class="form-group">
                      <div class="col-md-7 col-md-offset-2 col-sm-10">
                        ...
                        <a href="/ops/referencia/pdf/libretaPdf.php?provincia=<?php echo $_GET['prov']; ?>&fecha=<?php echo $diaMostrar; ?>&recuSel=<?php echo $recuSel; ?>" role="button" class="btn btn-info pull-right" target="_blank"><i class="fa fa-print"></i> Imprimir</a>						
                      </div>
                    </div>
                  </div>
                  <!-- /.box-footer -->
      ...
      <div class="box box-primary">
        <div class="box-header">
          ...
          <h3 class="box-title">Servicios para día: <strong> <?php echo cambiarFecha($diaMostrar); ?></strong></h3> <a href="libretaCopiar.php" target="_blank"> Copiar</a>
        ...
        <div class="box-body">
          ...
          <div class="box-body table-condensed no-padding">
            <table class="table table-hover aumText">
              ...
              <tbody class="todo-list" id="sortable" >
                <?php
                  $i = 1;
                ...
                ?>
                <tr id="elemento-<?php echo $id; ?>">
                  <td><?php  echo $i++; ?></td>
                  <td><a class="class_a_href" href="/ops/mostrar/editServ.php?iden=<?php echo $rwList['idSv']; ?>" target="_blank"><i class="fa fa-<?php echo $rwList['icono']; ?>"></i></a></td>
                  <td><?php echo substr($rwList['hora'], 0, 5); ?></td>
                  <td><?php echo $rwList['ciaNom']; ?></td>
                  <td><?php echo $rwList['nomSer']; ?></td>
                  <td><?php echo $rwList['nombre']; ?></td>
                  <td><?php if($selUlti == '1') { echo $ulText; } else { @ambComple($rwList['recurso'], $rwList['enfermero'], $rwList['medico'], $rwList['recuCorto']);} ?></td>
                  <td><?php echo $rwList['recoger']; ?></td>
                  <td><?php echo $rwList['locRec']; ?></td>
                  <td><?php echo @$rwList['trasladar']; ?></td>
                  <td><?php echo @$rwList['locTras']; ?></td>
                  <td><?php echo $rwList['obs']; ?> <?php if($selUlti == '1') { echo $obsText; } ?></td>
                  <td><div class="vhlist" id="vhIda-<?php echo $rwList['idSv'] ?>"><?php echo $rwList['matricula']; ?></div></td>
                </tr>
                <?php } ?>
              </tbody>
            </table>
