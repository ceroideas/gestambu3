...
if($numSqlIda == 0 && $numSqlVta == 0 ) {
  if($rwTabDem['cod_servicio'] > 60 && $rwTabDem['cod_servicio'] < 68) { // Servicio de enfermería
    if($rwSqlIda['estado'] > 5 ) { // Creado como anulado
      //$calI_V = "2";
      $estRes = "15";
      $menObs = "";
      $hDCsta = 1;
      ...
      $barraEst = "Servicio Anulado";
    } elseif($rwSqlIda['estado'] == 5 ) {
      //$calI_V = "2";
      $estRes = "14"; //Servicio finalizado
      $menObs = "";
      $hDCsta = 1;
      $mosVta = 0;
      $mensaLog = "Servicio creado como finalizado";
      $barraEst = "Creado como finalizado";
...
} elseif($numSqlIda == 1 && $numSqlVta == 0) {
  //Hay registro de ida pero no de vuelta
  if($rwTabDem['cod_servicio'] > 89 && $rwTabDem['cod_servicio'] < 119 ) {
  ...
  } elseif($rwTabDem['cod_servicio'] == 120 || $rwTabDem['cod_servicio'] == 130) { // Uvi primaria - no tiene vuelta
    if($rwSqlIda['estado'] > 5 ) { // El servicio no tiene vuelta y la ida esta anulada
      $calI_V = "2";
      $estRes = "15";
      $hDCsta = 0;
  ...
  if($rwTabDem['cod_servicio'] > 89 && $rwTabDem['cod_servicio'] < 119 ) {
    if($rwSqlIda['estado'] > 5 && $rwSqlVta['estado'] > 5) { // Anulada ida y vuelta
      ...
      $hDCsta = 1;
      $mosVta = 0;
      $mensaLog = "Servicio creado vuelta anulada";
  ...
  } elseif($rwTabDem['cod_servicio'] == 120 || $rwTabDem['cod_servicio'] == 130 ) { // Uvi primaria - no tiene vuelta - lo trata como ida y vuelta al tener registro de ida y de vuelta
    if($rwSqlIda['estado'] > 5 && $rwSqlVta['estado'] > 5) { // Anulada ida y vuelta
      $calI_V = "1";
      $estRes = "15";
      $menObs = "ANULADO";
  ...
  if($rwTabDem['cod_servicio'] > 89 && $rwTabDem['cod_servicio'] < 119 ) {
  ...
  } elseif($rwTabDem['cod_servicio'] == 120 || $rwTabDem['cod_servicio'] == 130 ) { //
    ...
    if($rwSqlVta['estado'] > 5 ) { // El servicio no tiene ida, la vuelta esta anulada . - son servicios únicos
      $calI_V = "3";
      $estRes = "15";
      $menObs = "ANULADO";
      $hDCsta = 1;
