  ...
  if($countDNI > 3) {
    ...
    if($numDNI == 1) {
      ...
      if($countPoli > 3) {
        ...
        if($numPoli == 1) {
          ...
          $compPac = mysqli_query($gestambu, "SELECT poliza, idPac, pNombre, pApellidos, idCia, fallecido
            FROM paciente
            WHERE poliza LIKE '%$poliza%' AND pApellidos LIKE '%$apellidos%' AND pNombre LIKE '%$nombre%' AND idCia ='$cia' AND fallecido !='2'");
          ...
          if($numPac == 1) {
            ...
            if($countApe == 0) {
              ...
              //Crea nuevo paciente
              $insPac = "INSERT INTO paciente (idCia, pacDNI, poliza, pNombre, pApellidos, edad, sexo, delegacion, tlf1, tlf2, direccion, localidad, provincia)
                VALUES ('$cia', '$DNI', '$poliza', '$nombre', '$apellidos', '$edadTab', '$sexo', '$deleg', '$tlf1', '$tlf2', '$recoger', '$locRec', $prov)
                ";
              if(mysqli_query($gestambu, $insPac)) {
                //Devuelve el id del último registro insertado en la tabla
                $pacienteID = mysqli_insert_id($gestambu);
              } else {
                $pacienteID = "";
                echo "Error: " . $insPac . "<br>" . mysqli_error($gestambu);
              }
            }
          }
        } else {
        ...
        //Se ha de hacer 3 comprobaciones más para ver si el paciente se corresponde - nombre, apellidos y compañía
        $compPac = mysqli_query($gestambu, "SELECT idPac, pNombre, pApellidos, idCia, fallecido
          FROM paciente
          WHERE pApellidos LIKE '%$apellidos%' AND pNombre LIKE '%$nombre%' AND idCia ='$cia' AND fallecido !='2'");
        $rwPac = mysqli_fetch_assoc($compPac);
        $numPac = mysqli_num_rows($compPac);
        //Devuelve los valores de comprobación
        if($numPac == 1) {
    ...
    if($countPoli > 3 ){
      ...
      if($numPoli == 1) {
        ...
        $compNomAp = mysqli_query($gestambu, "SELECT idPac, pNombre, pApellidos, idCia, fallecido
          FROM paciente
          WHERE pApellidos LIKE '%$apellidos%' AND pNombre LIKE '%$nombre%' AND idCia ='$cia' AND fallecido !='2'");
        ...
        if($numCompNomAp > 0) { //PENDIENTE DE REPARAR - ESTO ES PROVISIONAL 
          $okPac = 1;
