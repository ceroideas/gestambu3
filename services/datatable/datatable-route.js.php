<script>
  const keysRoute = JSON.parse('<?php echo $keysRoute ?>');
  $(function() {
    ...
    import('/docs/plugins/datatables/jquery.dataTables.min.js').then(_=>{
      ...
      import('/docs/plugins/datatables/dataTables.bootstrap.min.js').then(__=>{
        ...
        let table = '#table-' + idTableRoute;
        urlRoute = urlRoute.replace('/gestambu/www', '');
        ...
        $(table).DataTable({
          ...
          'serverMethod': 'post',
          'ajax': {
            'url': urlRoute+'?prov=<?= isset($_GET['prov']) ? $_GET['prov'] : 29 ?>&final=<?= isset($esp) && $esp != 0 ? $esp : "" ?>&filFecha=<?= isset($filFecha) ? $filFecha : "" ?>',
            'data': {
              fIni: $('[name=fIni]').val(),
              fFin: $('[name=fFin]').val()
            }
