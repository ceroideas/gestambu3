<script>
  ...
  const keysAmbulance = JSON.parse('<?php echo $keysAmbulance ?>');
  $(function() {
    ...
    import('/docs/plugins/datatables/jquery.dataTables.min.js').then(_=>{
      ...
      import('/docs/plugins/datatables/dataTables.bootstrap.min.js').then(__=>{
        ...
        let table = '#table-' + idTableAmbulance; 
        urlAmbulalnce = urlAmbulalnce.replace('/gestambu/www','');
        ...
        $(table).DataTable({
          ...
          'serverMethod': 'post',
          'ajax': {
            'url': urlAmbulalnce+'?prov=<?= isset($_GET['prov']) ? $_GET['prov'] : 29 ?>&final=<?= isset($esp) && $esp != 0 ? $esp : "" ?>&filFecha=<?= isset($filFecha) ? $filFecha : "" ?>',
          },
          'columns': keysAmbulance,
          "language": {
