<script>
  const keysMedic = JSON.parse('<?php echo $keysMedic ?>');
  $(function() {  
    ...
    import('/docs/plugins/datatables/jquery.dataTables.min.js').then(_=>{
      ...
      import('/docs/plugins/datatables/dataTables.bootstrap.min.js').then(__=>{
        ...
        let table = '#table-' + idTableMedic; 
        urlMedic = urlMedic.replace('/gestambu/www','');
        ...
        $(table).DataTable({
          ...
          'serverMethod': 'post',
          'ajax': {
            'url': urlMedic+'?prov=<?= isset($_GET['prov']) ? $_GET['prov'] : 29 ?>&final=<?= isset($esp) && $esp != 0 ? $esp : "" ?>&filFecha=<?= isset($filFecha) ? $filFecha : "" ?>',
            'data': {
              fIni: $('[name=fIni]').val(),
              fFin: $('[name=fFin]').val()
            }
